...
final class FunctionsTest extends TestCase {
    ...
    #[Test]
    public function test_requireIfExists_helper_require_once_a_file_if_exists(): void { 
        $file = vfsStream::newFile('test.txt')->at($this->filesystem_root)->setContent('test');

        \Safe\ob_start();
        foreach(range(1, 5) as $i){
            requireIfExists($file->url(), use_require_once: true);
        }
        $content = ob_get_clean();
    ...
    #[Test]
    public function test_requireIfExists_helper_require_a_file_if_exists(): void { 
        $file = vfsStream::newFile('test.txt')->at($this->filesystem_root)->setContent('test');

        \Safe\ob_start();
        foreach(range(1, 5) as $i){
            requireIfExists($file->url(), use_require_once: false);
        }
        $content = ob_get_clean();
    ...
    #[Test]
    public function test_requireIfExists_helper_does_not_require_once_a_file_if_it_does_not_exists(): void { 
        \Safe\ob_start();
        foreach(range(1, 5) as $i){
            requireIfExists('test', use_require_once: true);
        }
        $content = ob_get_clean();
    ...
    #[Test]
    public function test_requireIfExists_helper_does_not_require_a_file_if_it_does_not_exists(): void { 
        \Safe\ob_start();
        foreach(range(1, 5) as $i){
            requireIfExists('test', use_require_once: true);
        }
